...
test('should create product publish document success', () => {
  const processTest = () => {
    ...
    let params = InterfaceClientParamsCreatePublishDoc.create(APP_ID, DOC_PUBLISH_ID, MODEL_ID, PRODUCT_ID, sub.hash('hello'), '50', '40');
...
test('should create product identify document success', () => {
  const processTest = () => {
    ...
    let params = InterfaceClientParamsCreateIdentifyDoc.create(APP_ID, DOC_IDENTIFY_ID, PRODUCT_ID, sub.hash('hello'), '100', '23', '78', 'C0001' + new Date().getTime());
...
test('should create product try document success', () => {
  const processTest = () => {
    ...
    let params = InterfaceClientParamsCreateTryDoc.create(APP_ID, DOC_TRY_ID, PRODUCT_ID, sub.hash('hello'), '200', '99', '87', 'C0001' + new Date().getTime());
...
test('should power compute correct', () => {
  const processTest = () => {
    ...
    sub.unlock(accountA.json.address, pwd);
    ...
    const prepareFees = async () => {
      await sub.devTransfer(accountA.json.address, '1000');
    ...
    const createModel = async () => {
      let client_params = InterfaceClientParamsCreateModel.create(app_id, 'E0001', 'name1', model_type, sub.hash('hello'));
      let user_sign = sub.paramsSign(ClientParamsCreateModel, client_params, accountA.json.address);
      let auth_params = InterfaceAuthParamsCreateModel.create(model_id);
      let auth_sign = sub.paramsSign(AuthParamsCreateModel, auth_params, sub.getDevAdmin().address);

      return sub.createModel(client_params, auth_params, accountA.json.address, user_sign, sub.getDevAdmin().address, auth_sign);
    };

    const createPubDoc = () => {
      let params = InterfaceClientParamsCreatePublishDoc.create(app_id, pub_doc_id, model_id, product_id, sub.hash('hello'), '95', '0');
      let user_sign = sub.paramsSign(ClientParamsCreatePublishDoc, params, accountA.json.address);
      let auth_sign = sub.paramsSign(ClientParamsCreatePublishDoc, params, sub.getDevAdmin().address);

      return sub.createDocument(0, params, accountA.json.address, user_sign, sub.getDevAdmin().address, auth_sign);
    };

    const createIdentifyDoc = () => {
      let params = InterfaceClientParamsCreateIdentifyDoc.create(app_id, identify_doc_id, product_id, sub.hash('hello'), goods_price, '0', '0', cart_id);
      let user_sign = sub.paramsSign(ClientParamsCreateIdentifyDoc, params, accountA.json.address);
      let auth_sign = sub.paramsSign(ClientParamsCreateIdentifyDoc, params, sub.getDevAdmin().address);
      return sub.createDocument(1, params, accountA.json.address, user_sign, sub.getDevAdmin().address, auth_sign);
    };

    const createTryDoc = () => {
      let params = InterfaceClientParamsCreateTryDoc.create(app_id, try_doc_id, product_id, sub.hash('hello'), goods_price, '0', '0', cart_id);
      let user_sign = sub.paramsSign(ClientParamsCreateTryDoc, params, accountA.json.address);
      let auth_sign = sub.paramsSign(ClientParamsCreateTryDoc, params, sub.getDevAdmin().address);
      return sub.createDocument(2, params, accountA.json.address, user_sign, sub.getDevAdmin().address, auth_sign);
    };
    ...
    const commentPub = async () => {
      ...
      owner_sign = sub.paramsSign('CommentData', c, accountA.json.address);
      auth_sign = sub.paramsSign('CommentData', c, sub.getDevAdmin().address); 
      await sub.createComment(c, accountA.json.address, owner_sign, sub.getDevAdmin().address, auth_sign);
    };

    const commentTry = async () => {
      let c = InterfaceComment.create(app_id, try_doc_id, 'C0003' + ts, sub.hash('comment'), "1", "0");
      let owner_sign = sub.paramsSign('CommentData', c, accountA.json.address);
      let auth_sign = sub.paramsSign('CommentData', c, sub.getDevAdmin().address); 
      await sub.createComment(c, accountA.json.address, owner_sign, sub.getDevAdmin().address, auth_sign);
    };
...
test('should set app admin success', () => {
  const processTest = () => {
    return sub.membersSetAppAdmin(APP_ID, sub.getDevAdmin().address);
  };
  ...
  const processTest = () => {
    ...
    const userExchangeConfirm = () => {
      ...
      const confirm = () => {
        ...
        return sub.appFinancedUserExchangeConfirm(obj, sub.getDevAdmin().address);
      };
...
test('should model cycle income test success', () => {
  const processTest = () => {
    ...
    // give some fees
    const prepareFees = () => sub.devTransfer(testJson.address, '1000');
