...
test('should app finance success', () => {
  const processTest = () => {
    ...
    const addFinanceMember = async () => {
      ...
      let signObj = sub.createSignObject('FinanceMemberParams', {deposit: "10000", member: testJson.address});
      let sign = sub.paramsSign('FinanceMemberParams', signObj, testJson.address);

      let result = await sub.membersAddFinanceMember(signObj, sub.getDevAdmin().address, testJson.address, sign);
      expect(result.hash).toBeDefined();
    };
    ...
    const userExchangeSubmit = () => {
      ...
      return sub.appFinancedUserExchangeRequest(params, exchangeUser.address, user_sign, testJson.address, auth_sign)
        .then(result => {
          expect(result.hash).toBeDefined();
          expect(result.finance_member).toBe(testJson.address);
        });
    };
