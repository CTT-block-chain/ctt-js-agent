...
test('should app finance success', () => {
  const processTest = () => {
    ...
    const sign = sub.sign(testJson.address, 'fake');
    const finFundAddress = '5EYCAe5ijiYfyeTn5e8qoUTDJTVY5HBXJjVN7RDe4RvCAAcU';
    const exchangeUserInitalBalance = '10000';
    ...
    let exchangeUser;
    let exchangeUserHistoryBalance;
    ...
    // prepare a new test acccount for user exchange, we need to prepare some initial funding for it
    const prepareExchangeUser = () => {
      exchangeUser = sub.newAccount('exchange').json;
      return sub.devTransfer(exchangeUser.address, exchangeUserInitalBalance);
    }
    ...
    // sudo action
    const registerApp = () => sub.sudoAddApp("test", "commodity_general", testJson.address, testJson.address, 1)
      .then(data => {
    ...
    const performAppFinanced = () => {
      return sub.sudoAppFinance(app_id, proposalId, kptAmount, exchange, testJson.address, sign, testJson.address, sign)
        .then(result => {
          expect(result.hash).toBeDefined();
        });
    };
    ...
    const verifyUserHistoryBalance = () => {
      ...
      return giveUserSome()
        ...
        .then(result => {
          ...
          // expect this balance still be exchangeUserInitalBalance
          console.log(`history:{} init:{}`, historyBalance.toString(), exchangeUserInitalBalance);
          expect(historyBalance.cmp(sub.convertBalance(exchangeUserInitalBalance))).toBe(0);
        });
    }
    ...
    const userExchangeSubmit = () => sub.appFinancedUserExchangeRequest(app_id, proposalId, exchangeUserPortion, exchangeUser.address, sign, testJson.address, sign)
      .then(result => {
        expect(result.hash).toBeDefined();
      });
    ...
    const checkUserExchangeLocked = () => {
      ...
      const checkLock = (result) => {
        ...
        console.log("reerved:", reserved.toString());
    ...
    const userExchangeConfirm = () => {
      ...
      const confirm = () => sub.appFinancedUserExchangeConfirm(app_id, proposalId, exchangeUser.address, sign, testJson.address, sign);
    ...
    return prepareExchangeUser()        // 准备一个测试用户账户，给予一定初始账户余额作为赎回兑换时的参考历史余额
      ...
      .then(performAppFinanced)         // 执行融资提案（SUOD操作） 
