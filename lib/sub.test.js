...
const initApi = () => sub.initApi(apiAddr);
...
test("transfer should success", () => {
  const processTest = () => {
    ...
    return sub.devTransfer("5GrwX4JEmrmk2RM6aTorJxzbpDWzgoifKVtHCPdjQohjRPo6", 1000000000000000);
  };
...
test("should create model success", () => {
  const processTest = () => {
    ...
    let mod = model.create("B003", "G0001" + new Date().getTime(), "E0001", "M0001", sub.hash("Hello"));
    ...
    return sub.createModel(mod, sub.getDevAdmin().address, testSign, testJson.address, testSign);
  };
