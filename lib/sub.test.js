  ...
  const processTest = () => {
    ...
    const balanceBeforeAction = () => {
      return Promise.all([readFinFund, readInvestorFund]).then(results => {
        finFundBalanceBefore = results[0].transferable;
        investorBalanceBefore = results[1].transferable;
      });
    }
    ...
    const balanceAfterAction = () => {
      return Promise.all([readFinFund, readInvestorFund]).then(results => {
        finFundBalanceAfter = results[0].transferable;
        investorBalanceAfter = results[1].transferable;
      });
    }

    const checkAppFinanced = () => {
      expect(finFundBalanceBefore.sub(finFundBalanceAfter)).toBe(new BN(100000))
    }

    // test flow:
    return setAppAdmin()
      .then(addInvestorAccount)
      .then(balanceBeforeAction)
      .then(performAppFinanced)
      .then(balanceAfterAction)
      .then(checkAppFinanced);
  };
