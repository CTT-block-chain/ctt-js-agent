...
const { createKeyMulti, encodeAddress, sortAddresses, blake2AsHex, mnemonicGenerate, cryptoWaitReady, signatureVerify } = require('@polkadot/util-crypto');
...
const { AppFinancedProposalParams, AppFinancedUserExchangeParams, AppFinancedUserExchangeConfirmParams, CommentData,
  ...
  AppKeyManageParams, AppIncomeRedeemParams, AppIncomeRedeemConfirmParams, ModelKeyParams, FinanceMemberParams } = require('./signParamsDefine');
...
const appFinancedUserExchangeRequest = async (params, user_key, user_sign, server_key, server_sign) => {
  ...
  if (result && result.events) {
    for (let i = 0; i < result.events.length; i++ ) {
      ...
      if (event.section == 'kp' && event.method == 'AppFinanceUserExchangeStart') {
        result.finance_member = event.data[1].toString(); // finance member
        break;
      }
    }
  }
...
const appFinancedUserExchangeCompensate = async (sender_pub_key, app_id, proposal_id) => {
  isKeyringReady();
  isApiReady();

  let txInfo = {
    module: 'kp',
    call: 'appFinanceRedeemCompensate',
    pubKey: sender_pub_key,
  };

  const result = await sendTx(txInfo, [Number(app_id), proposal_id]);
  return result;
}
...
const appIncomeUserExchangeRequest = async (params, user_key, user_sign, server_key, server_sign) => {
  ...
  if (result && result.events) {
    for (let i = 0; i < result.events.length; i++ ) {
      let event = result.events[i];
      if (event.section == 'kp' && event.method == 'AppCycleIncomeRedeem') {
        result.finance_member = event.data[1].toString(); // finance member
        break;
      }
    }
...
const appIncomeUserExchangeCompensate = async (sender_pub_key, app_id, cycle) => {
  isKeyringReady();
  isApiReady();

  let txInfo = {
    module: 'kp',
    call: 'appIncomeRedeemCompensate',
    pubKey: sender_pub_key,
  };

  const result = await sendTx(txInfo, [Number(app_id), Number(cycle)]);
  return result;
}
...
const membersAddFinanceMember = async (params, sender_pub_key, app_user_key, app_user_sign) => {
  isKeyringReady();
  isApiReady();
  ...
  const result = await sendTx(txInfo, [params, app_user_key, app_user_sign]);
...
const membersFinanceMemberAddDeposit = async (sender_pub_key, deposit) => {
  isKeyringReady();
  isApiReady();
  
  const txInfo = {
    module: 'members',
    call: 'financeMemberAddDeposit',
    pubKey: sender_pub_key,
  };

  const result = await sendTx(txInfo, [convertBalance(deposit)]);
  console.log('membersFinanceMemberAddDeposit result:', result);
...
const paramsSign = (paramsType, interfaceObj, signer_address) => {
  ...
  switch (paramsType) {
    ...
    case FinanceMemberParams: {
      buf = InterfaceFinanceMemberParams.encode(interfaceObj);
      break;
    }
    default:
      break;
...
const createSignObject = (params_type, params_data) => {
  switch (params_type) {
    ...
    case FinanceMemberParams: {
      const {deposit, member} = params_data;
      return InterfaceFinanceMemberParams.create(deposit, member);
    }
    default:
...
function calcInflation (totalStaked, totalIssuance) {
  ...
  const stakedFraction = totalStaked.muln(1000000).div(totalIssuance).toNumber() / 1000000;
...
const multisig_test = async () => { 
  ...
  setupAccountByJson(testJson);
  unlock(testJson.address, '123456');

  //await devTransfer(testJson.address, "10");

  const addresses = [testJson.address, getDevAdmin().address];
  
  const multiAddress = createKeyMulti(addresses, 2);

  const Ss58Address = encodeAddress(multiAddress, SS58Prefix);
  console.log("multi-addr:", Ss58Address);

  let target_tx = this.api.tx['balances']['transfer'](testJson.address, convertBalance("10"));
  const { weight } = await target_tx.paymentInfo(getDevAdmin().address);

  let info = await this.api.query.multisig.multisigs('5Gg5H81H4jEPwm5PbtzFXjgd4gQH1QkdkrRjCtGMZks5YAg3', '0xe2545d521eecbf65efe842368a365e56bf27100b5da87307af31630156a93421');
 ...
 /*const txInfo = {
  ...
  const result1 = await sendTx(txInfo, [3, [testJson.address], info.when, target_tx.method.toHex(), false, weight]);
  console.log("step2:", JSON.stringify(result1));*/
...
module.exports = {
  ...
  appFinancedUserExchangeCompensate: appFinancedUserExchangeCompensate,
  ...
  appIncomeUserExchangeCompensate: appIncomeUserExchangeCompensate,
  ...
  membersFinanceMemberAddDeposit: membersFinanceMemberAddDeposit,
