...
const appFinancedUserExchangeRequest = async (appId, proposalId, amount, user_key, user_sign, server_key, server_sign) => {
  let txInfo = {
    module: 'kp',
  ...
  let params = {
    ...
    appId: Number(appId),
    proposalId,
    exchangeAmount: convertBalance(amount),
  };
...
// this function only could be invoked from trust server
const appFinancedUserExchangeConfirm = async (appId, proposalId, user_key, user_sign, server_key, server_sign) => {
  let txInfo = {
    module: 'kp',
...
const democracyAddApp = async (appAdd, user_key, user_sign, auth_key, auth_sign, deposit) => {
  isKeyringReady();
  isApiReady();

  let imageHash = createPreImage('kp', 'democracyAddApp', [appAdd, user_key, user_sign, auth_key, auth_sign]);
  console.log('imageHash:', imageHash);
...
const sudoAddApp = async (app_name, app_type, identity_key, admin_key, return_rate) => {
  isKeyringReady();
  isApiReady();

  let params = InterfaceAddAppParams.create(app_name, app_type, identity_key, admin_key, return_rate);
  let sign = paramsSign('AddAppParams', params, getDevAdmin().address);
  ...
  const result = await sendTx(txInfo, [params, getDevAdmin().address, sign, getDevAdmin().address, sign], true);
