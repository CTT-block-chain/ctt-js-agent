...
const addCommodityType = async (type_id, type_desc) => {
  return new Promise(async (resolve, reject) => {
    ...
    this.api.tx.sudo
      .sudo(this.api.tx.kp.createCommodityType(type_id, type_desc))
      .signAndSend(sudoPair, ({ status, events }) => {
        if (status.isFinalized) {
          console.log(status.asFinalized.toHex());
          events.forEach(async ({ phase, event: { data, method, section } }) => {
            console.log('createCommodityType:', section.toString(), method.toString(), data.toString());
            section = section.toString();
            method = method.toString();
            ...
            switch (section) {
              case 'system':
                ...
                break;
              case 'kp':
                if (method === 'CommodityTypeCreated') {
                  const datajson = JSON.parse(data.toString());
                  // resolve with event data
                  resolve({
                    tx: status.asFinalized.toHex(),
